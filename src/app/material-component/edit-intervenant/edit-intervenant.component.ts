...
import { Certification, Competence, Experience, Specialization } from 'src/app/Models/user';
...
export class EditIntervenantComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private userService: IntervenantService, private route: ActivatedRoute,private router:Router) { }
  ...
  ngOnInit(): void {
    ...
    this.editUserForm = this.formBuilder.group({
      ...
      // Ajoutez le reste des champs d'informations utilisateur ici
  ...
  getAgentMIssion(id: string): void {
    this.userService.getIntervenantById(id).subscribe(intervenant => {
  ...
  onSubmit(): void {
    if (this.editUserForm.valid && this.user && this.user._id) {
      ...
      this.userService.updateIntervenant(this.user._id, this.editUserForm.value).subscribe(
        response => {
          console.log('Soldier updated successfully:', response);
          Swal.fire({
            title: 'Success!',
            text: 'Soldier successfully updated.',
            icon: 'success',
        ...
        error => {
          ...
          Swal.fire({
            title: 'Error!',
            text: 'An error occurred while updating the  Soldier. Please try again.',
            icon: 'error',
            confirmButtonText: 'OK'
          });
...
return() {
  Swal.fire({
    ...
    text: 'You will be redirected to the Soldier page.',
    icon: 'question',
